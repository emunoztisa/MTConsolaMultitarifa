...
namespace TestMdfEntityFramework.Controllers
{
    public class OperadoresController
    {
        ...
        public List<ct_operadores> GetOperadores()
        {
            List<ct_operadores> list_temp = new List<ct_operadores>();

            Api<ResOperadores> servicio = new Api<ResOperadores>();
            Comun mc = new Comun();

            string base_url = mc.obtenerValorDeAppConfig("URL_BASE");
            string metodo_web = "mt/operadores";
            ...
            ResOperadores responseGET_withToken = servicio.RequestGet_withToken(base_url, metodo_web, headers, typeof(ResOperadores));
            ResOperadores resp = responseGET_withToken;

            //foreach (ResponseLugares.Data data1 in data)
            foreach (ct_operadores item in resp.data)
            {
                ct_operadores reg = new ct_operadores();
                reg.pkOperador = item.pkOperador;
